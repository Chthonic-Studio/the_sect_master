...
# Ordered lists for character culture dropdown (Page 1) — unchanged
...
# Order matches regions.json; must map 1-to-1 with REGION_LABELS.
...
%SectCultureDescLabel.text = Definitions.CULTURE_DESCRIPTIONS.get(culture_key,
"A culture with its own unique martial traditions.")
...
# 2. Generate the world's rival sects (province-based system)
