...
func _populate_header() -> void:
	...
	spiritual_root_icon.tooltip_text = Definitions.get_spiritual_root_name(current_character.spiritual_root)
	...
	if current_character is CultivatorResource:
		...
		if realm:
			...
			cultivation_realm_icon.tooltip_text = realm.display_name
		else:
	...
	_populate_trait_icons(traits_container, current_character)
...
# REASON FOR CHANGE:
# Added a reusable helper function to avoid duplicating code between this script
# and the PlayerCharacterMenu script.
