...
## Panel that shows detailed information about a region.
## Tabs: Overview | Sects | Actions
## Opened by clicking a region on the map (REGIONS layer).
...
var _province_count_label: Label
...
## Called by UIManager when the panel is opened with a region_id payload.
func setup_panel(region_id: Variant = null) -> void:
	if region_id == null or not (region_id is String):
		return
	_region_id = region_id as String
	_refresh()

func _refresh() -> void:
	var reg_data: Dictionary = DataManager.regions_registry.get(_region_id, {})
	if reg_data.is_empty():
		return

	var reg_name: String = reg_data.get("name", _region_id)
	var culture_key: String = reg_data.get("culture", "CENTRAL_PLAINS")
	var culture_name: String = culture_key.replace("_", " ").capitalize()
	var description: String = reg_data.get("description", "No description available.")

	var provinces: Array = MapManager.get_provinces_in_region(_region_id)
	var sects: Array = MapManager.get_sects_in_region(_region_id)

	_title_label.text = reg_name
	_culture_label.text = "Culture: %s" % culture_name
	_province_count_label.text = "Provinces: %d" % provinces.size()
...
func _make_sect_row(sect: SectData) -> Control:
	...
	prov_lbl.custom_minimum_size = Vector2(100, 0)
	prov_lbl.add_theme_color_override("font_color", Color(0.7, 0.7, 0.7))
	hbox.add_child(prov_lbl)
...
func _build_ui() -> void:
	...
	btn_close.pressed.connect(func(): UIManager.close_panel("region_view"))
	...
	_culture_label.text = "Culture: —"
	info_row.add_child(_culture_label)

	_province_count_label = Label.new()
	_province_count_label.text = "Provinces: 0"
	info_row.add_child(_province_count_label)
...
func _build_overview_tab() -> void:
	...
	header.text = "Provinces in this Region"
	...
	_provinces_list = VBoxContainer.new()
	_provinces_list.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_provinces_list.add_theme_constant_override("separation", 2)
	scroll.add_child(_provinces_list)
...
func _build_sects_tab() -> void:
	...
	_make_sort_button("Strength",   SortMode.STRENGTH,      sort_row)
	_make_sort_button("Size",       SortMode.SIZE,          sort_row)
	_make_sort_button("Relation",   SortMode.RELATIONSHIP,  sort_row)
	_make_sort_button("Name",       SortMode.NAME,          sort_row)

	# Column headers
	var header := HBoxContainer.new()
	tab.add_child(header)
	_add_header_label(header, "Name",      true)
	_add_header_label(header, "Province",  false, 100)
...
func _build_actions_tab() -> void:
	...
	_tab_container.add_child(tab)
	var lbl := Label.new()
	lbl.text = "Region actions will be available in a future update."
